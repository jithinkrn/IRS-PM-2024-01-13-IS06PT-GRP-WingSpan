...
def plot_feature_importances(rf_classifier, features):
    ...
    plt.title('Feature Importances in Bird Location Prediction')
...
X = df[['Species_encoded', 'Day_of_Year', 'Month', 'Day_of_Week', 'tempmax']]
...
current_directory_path = os.getcwd()
model_path = os.path.join(current_directory_path + "/Models/", 'rf_location_classifier.joblib')
...
target_names = [location_encoder.inverse_transform([cls])[0] for cls in unique_classes]
